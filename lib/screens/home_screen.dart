...
class HomeScreenState extends State<HomeScreen> implements HomeScreenPaintSelection {
  ...
  late final List<Widget> _screens = [
    ...
    const ExploreScreen(),
    SearchScreen(onPaintSelectedForRoller: onPaintSelectedFromSearch), // Pass callback
  ...
  void onPaintSelectedFromSearch(Paint paint) {
    ...
    Debug.info('HomeScreen', 'onPaintSelectedFromSearch', 'Switched to roller tab, scheduling dialog');
  ...
  void _showColumnSelectionDialog(Paint paint) {
    Debug.info('HomeScreen', '_showColumnSelectionDialog', 'Showing color selection dialog for: ${paint.name}');
    
    final rollerState = _rollerKey.currentState;
    if (rollerState == null) {
      Debug.error('HomeScreen', '_showColumnSelectionDialog', 'RollerState is null, cannot show dialog');
      return;
    }

    final paletteSize = rollerState.getPaletteSize();
    Debug.info('HomeScreen', '_showColumnSelectionDialog', 'Palette size: $paletteSize');
          ...
          children: [
            ...
            const SizedBox(height: 16),
                    ...
                    Align(
                      ...
                      child: Text(
                        rollerState.canAddNewColor() ? 'Or choose position to replace:' : 'Choose position to replace:',
                        style: Theme.of(context).textTheme.titleSmall?.copyWith(
                            ...
                            children: [
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    Text(
                                      currentPaint?.name ?? 'Empty Position ${index + 1}',
                                      style: Theme.of(context).textTheme.titleSmall?.copyWith(
                                        fontWeight: FontWeight.w500,
                                      ),
                                    ),
