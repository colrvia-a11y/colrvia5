...
class DebugLogger {
  ...
  void logSetState(String widgetName, String location, {String? details}) {
    ...
    if (count % 50 == 0) {
      debugPrint('📊 setState Stats: $widgetName at $location has called setState $count times');
    }
  }
  ...
  void logBuild(String widgetName, String location, {String? details}) {
    ...
    if (count % 100 == 0) {
      debugPrint('🔨 Build Stats: $widgetName at $location has built $count times');
    }
  }
  ...
  void logMediaQuery(String widgetName, String location, String property) {
    ...
    if (count > 100) {
      debugPrint('📱 MediaQuery Overuse: $widgetName at $location accessing $property $count times');
    }
  }
  
  void logPostFrameCallback(String widgetName, String location, {String? details}) {
    if (!_enabled) return;
    ...
    if (count > 50) {
      debugPrint('⏰ PostFrameCallback Overuse: $widgetName at $location called $count times');
      if (details != null) debugPrint('   Details: $details');
    }
  }
  
  void logError(String widgetName, String location, String error, {String? stackTrace}) {
    if (!_enabled) return;
...
class Debug {
  ...
  static void postFrameCallback(String widgetName, String location, {String? details}) {
    DebugLogger.instance.logPostFrameCallback(widgetName, location, details: details);
  }
  
  static void error(String widgetName, String location, String error, {String? stackTrace}) {
    DebugLogger.instance.logError(widgetName, location, error, stackTrace: stackTrace);
  }
