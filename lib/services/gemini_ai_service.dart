...
class GeminiAIService {
  static const String _apiKey = 'YOUR_GEMINI_API_KEY'; // Configure in environment
    ...
    final response = await http.post(
      ...
      body: jsonEncode({
        'contents': [{
          'parts': [
            {
              'text': '''Analyze this interior/exterior space image and provide a JSON response with:
              {
  ...
  static Future<Uint8List> transformSpace({
    ...
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
      final imageData = data['candidates'][0]['content']['parts'][0]['inline_data']['data'];
