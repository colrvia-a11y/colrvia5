...
class FirebaseService {
  ...
  static Stream<model.ColorStory> storyStream(String id) {
    ...
    debugPrint('🐛 FirebaseService: TEMPORARILY BYPASSING AUTH STATE - making direct query');
    ...
    return _db.collection('colorStories').doc(id).snapshots().map((s) {
      ...
      debugPrint('🐛 FirebaseService: Current user at query time = ${currentUser?.uid}');
      ...
      if (s.exists) {
        ...
        if (data.isNotEmpty) {
          ...
          debugPrint('🐛 FirebaseService: Story ownerId = ${data['ownerId']}, current user = ${currentUser?.uid}');
  ...
  static Future<List<Paint>> searchPaints(String query) async {
    ...
    try {
      ...
      results.addAll(nameSnapshot.docs.map((doc) => Paint.fromJson(doc.data(), doc.id)));
      ...
      results.addAll(brandSnapshot.docs.map((doc) => Paint.fromJson(doc.data(), doc.id)));
      ...
      results.addAll(codeSnapshot.docs.map((doc) => Paint.fromJson(doc.data(), doc.id)));
      
      // Search by hex (if query looks like hex)
      if (query.startsWith('#') || RegExp(r'^[0-9A-Fa-f]+$').hasMatch(query)) {
        String hexQuery = query.startsWith('#') ? query.toUpperCase() : '#${query.toUpperCase()}';
        final hexSnapshot = await _db.collection('paints')
          .where('hex', isEqualTo: hexQuery)
          .limit(10)
          .get();
          
        results.addAll(hexSnapshot.docs.map((doc) => Paint.fromJson(doc.data(), doc.id)));
      }
      ...
      if (results.isEmpty) {
        ...
        final broadSnapshot = await _db.collection('paints')
          .limit(100)
          .get();
          
        final allPaints = broadSnapshot.docs.map((doc) => Paint.fromJson(doc.data(), doc.id));
        results = allPaints.where((paint) {
  ...
  static Future<List<Paint>> _searchSamplePaints(String query) async {
    try {
      ...
      final samplePaints = await SamplePaints.getSamplePaints();
      final lowercaseQuery = query.toLowerCase().trim();
      
      return samplePaints.where((paint) {
        final name = paint.name.toLowerCase();
        final brand = paint.brandName.toLowerCase();
        final code = paint.code.toLowerCase();
        final hex = paint.hex.toLowerCase();
